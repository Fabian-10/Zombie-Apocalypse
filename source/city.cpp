...
void City::Ignorant_to_alerted(Denizen *b)
{
    Denizen *a;
...
void City::residents_move()
{
    for (int i = 0; i < residents.size() / 8; i++)
    {
        ...
        residents[r]->change_district(residents[r], district[n]);
    }
}
...
void City::simulate()
{
    ...
    for (int i = 0; i < 28; i++)
    {
        ...
        for (int j = 0; j < residents.size(); j++)
        {
            ...
            if (!check)
            {
                Ignorant_to_alerted(residents[j]);
            }
        }
